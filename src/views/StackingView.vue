    ...
    <AvaCard bg-color="#FFFFFF" class="mb-16">
      <div class="stacking__card-info">
        <div class="content">
          ...
          <div>
            <div class="content-title">{{ stake.my?.sum ?? 0 }}</div>
            <div class="content-subtitle">My staking</div>
          </div>
        </div>
        <div class="content">
          <div>
            <div class="content-title">{{ stake.my?.all_time_sum ?? 0 }}</div>
        ...
        <div v-if="stake.my?.staked_at" class="content">
          <div>
            ...
            <div class="content-subtitle grey">{{ moment(stake.my?.staked_at).format('DD.MM.YYYY') }}</div>
          </div>
        </div>
      </div>
    </AvaCard>

    <div class="stacking__header">
      Staking statistics
    </div>
    <div class="stacking__statistics">
      <div class="stacking__statistics__item">
        <div class="stacking__statistics__item-title">{{ stake.stats?.percent ? ('≈' + Math.floor(stake.stats?.percent).toLocaleString() + '%') : '∞' }}</div>
        <div class="stacking__statistics__item-subtitle">Pool APR 24h</div>
      </div>
      <div class="stacking__statistics__item">
        <div class="stacking__statistics__item-title">{{ stake.stats?.percent ? ('≈' + Math.floor(stake.stats?.percent * 7).toLocaleString()) + '%' : '∞' }}</div>
        <div class="stacking__statistics__item-subtitle">Pool APR 7d</div>
      </div>
      <div class="stacking__statistics__item">
        <div class="stacking__statistics__item-title">{{ stake.stats?.percent ? ('≈' + Math.floor(stake.stats?.percent * 180).toLocaleString()) + '%' : '∞' }}</div>
    ...
    <div class="stacking__statistics-users">
      <div class="stacking__statistics-users__item">
        ...
        <div class="stacking__statistics-users__item-subtitle flex align-center gap-5">
          <span class="mt-2">{{ stake.stats?.all_stakes_sum?.toLocaleString('en-EN') || '0' }}</span>
          <img v-if="stake.stats?.all_stakes_sum" src="@/assets/images/mini-coin.png" alt="coin">
        </div>
      </div>
      <div class="stacking__statistics-users__item">
        <div class="stacking__statistics-users__item-title">
          Users in staking
        </div>
    ...
    <AvaCard bg-color="#FFFFFF" class="mb-16">
      <div class="stacking__card-info justify-between">
        <div class="content">
          ...
          <div>
            <div class="content-title">{{ Math.floor(root.user?.tokens ?? 0).toLocaleString('en-En') }}</div>
